...
export class UberMapPage {
  ...
  filterByFilterObj() {
    ...
    const withMarker = _.filter(this.trips, (x) => x.marker)
    const filtered = FilterTimeProvider.filterbyTime(withMarker, this.filter);
    const results = _.map(filtered, 'marker')
